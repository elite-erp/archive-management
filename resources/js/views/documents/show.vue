    ...
    <div class="w-3/12 flex flex-col items-center gap-4">
      <div class="flex flex-row justify-center items-center">
        <label class="flex flex-row relative w-36 h-36">
          <img
            :src="document.photo"
            class="edit-document-photo h-36 w-36 rounded-full ring-4 object-cover ring-gray-200"
          />
          <input
            type="file"
            ...
            accept="image/*"
            @input.prevent="
              (e) => uploadPhoto(e, '.edit-document-photo', `/documents/${document.id}`)
            "
          />
          <button
            class="btn-icon absolute bg-green-100 top-24 left-0 shadow-md"
            type="button"
            @click.prevent="fileSelector('.edit-document-photo')"
          >
